...
module Plezi
  ...
  def app
    ::Plezi::Base::Autostart.turn_off
    puts "\n\nPlezi is feeling optimistic running version #{::Plezi::VERSION}." if @app.nil?
    @app ||= ::Plezi::Base::Router
  end

  # adds middleware
  def middleware(middleware, *args, &block)
    @app = middleware.new((@app ||= ::Plezi::Base::Router), *args, &block) # can't use Plezi.app or we'll disable Autostart
  ...
  def route(path, controller = nil, &block)
    if controller == :client
      ...
      controller = nil
      block = proc { Plezi.cache_needs_update?(client_path) ? Plezi.reload_file(client_path) : Plezi.load_file(client_path) }
    end
  ...
  # Disables Plezi's "smart" autostart feature, for when it's less smart...
  def no_autostart
    ::Plezi::Base::Autostart.turn_off
  end
end
