  ...
  module Base
    ...
    module Router
      ...
      class Host
        ...
        def update
          @params[:assets_public_regex] = /^#{@params[:assets_public].to_s.chomp('/')}\//i
          @params[:assets_public_length] = @params[:assets_public].to_s.chomp('/').length + 1
          @params[:assets_refuse_templates] = /(#{AssetManager.all_extentions.join('|')}|\.\.\/)$/i
          self
        end
      end
      ...
      def add_host(host_name, params = {})
        ...
        Iodine::Rack.public = params[:public] if params[:public] # the public folder is globaly shared.
      ...
      def add_shared_route(path, controller, &block)
        ...
        @hosts.each { |_n, h| h.routes << ::Plezi::Base::Router::Route.new(path, controller, &block) }
      end
      ...
      def url_for(controller, dest, params = {})
        ...
        host.routes.each do |r|
          ...
          next unless r.controller == controller
          ...
          r.url_array.each do |sec|
            ...
            if param_name && dest[param_name]
              ...
              raise ArgumentError, "URL can't be formatted becuse a required parameter (#{param_name}) isn't specified and it requires a special format (#{::Plezi::Base::Router::Route::REGEXP_FORMATTED_REQUIRED_PARAMS.match(sec)[2]})."
            end
          end
      ...
      def send_static_file(request, _response)
